...
test('AI: extended success path (payload.extra) shows payload with extra', async ({ page }) => {
  try {
    await page.goto(BASE + '/public/test-success-extended.html', { timeout: 5000 });
  } catch {
  ...
  expect(payload.result.extra).toBeDefined();
  expect(payload.result.extra.note).toBe('extended payload');
});
